...
class CautiousRandomForest:
    def __init__(self, n_trees=100, s=2, min_samples_leaf=1,combination='cdm-vote', discount_type='u65', risk_threshold=0.2, random_state=42):
    ...
    def __cr(self, probabilities):
        class_order = np.argsort(-probabilities)
        top_k = 0
        for k in range(1,self.n_class+1):
            if np.sum(probabilities[class_order[0:k]]) >= 1 - self.risk_threshold:
                return list(self.classes[class_order[0:k]])
            else:
                continue
    ...
    def __cdm_vote(self, intervals):
        ...
        if prediction_index is None:
            return self.__cdm_ave(intervals)
        else:
            return self.classes[list(prediction_index)]
    ...
    def predict(self, X, dacc=None):
        ...
        if self.combination == 'cr':
            all_proabilities = self.rf.predict_proba(X)
            for i in range(n_instance):
                predictions.append(self.__cr(all_proabilities[i]))
            return predictions
